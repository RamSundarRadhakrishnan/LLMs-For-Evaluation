...
### Running the Hybrid Architecture POC

The Hybrid Architecture POC combines two evaluation layers:

1. A Natural Language Inference cross-encoder checks whether each rubric point is supported by the student answer.
2. An LLM uses the rubric evidence, question, answer, answer key, subject, board, grade, and marks to produce the final structured evaluation.
...
```bash
pip install -U openai sentence-transformers numpy torch
```
...
```bash
cd HybridArchitecturePOC
python Evaluation.py
```

The script reads sample inputs from:

```text
sample_eval_data.json
```
...
The current POC uses the NLI cross-encoder as supporting rubric evidence, not as the final grader. Final grading and feedback are generated by the LLM.

The cross-encoder is currently configured to run on CUDA. If you are running on a CPU-only machine, update the device setting in `CrossEncoderEvaluator.py` before running the script.
